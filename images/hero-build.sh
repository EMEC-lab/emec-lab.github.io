...
#   START 는 리빌(모터가 켜지는 순간)보다 1.2초 앞에서 잡는다.
#     앞 0.6초는 크로스페이드에 먹히므로, 0.5초만 앞당기면 리빌 직전이
#     전환에 묻혀 안 보인다. 1.2초여야 0.6초가 눈에 남는다
#     리빌 시점  클립2 ~3.3s · 클립3 ~2.7s · 클립4 ~2.1s · 클립6 ~2.5s
#   인트로 2초 · 클립 2~6 은 4초 · 클립 7 만 4.8초
#   클립 7 은 팔(원본 ~4.0초까지)과 도시 복귀(원본 5.0초부터)를 둘 다
#     담아야 해서 길다. 끝을 앞당기면 도시가 잘려 루프 이음새가 깨진다
#   START+LEN 이 원본 길이(6.04초)를 넘지 않게 할 것
declare -A START=( [1]=0.0 [2]=2.0 [3]=1.5 [4]=0.9 [5]=0.3 [6]=1.3 [7]=1.2 )
declare -A LEN=(   [1]=2.0 [2]=4.0 [3]=4.0 [4]=4.0 [5]=4.0 [6]=4.0 [7]=4.8 )
