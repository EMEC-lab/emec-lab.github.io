...
- **국제 학술지**만 `impact` 에 Impact Factor 를 적습니다. 국내 학술지는 비워 둡니다.
  ...
  ```javascript
  impact: "8.9",
  jcrTop: "3.3", jcrRank: "6/184", jcrQuartile: "Q1",
  ```

  화면에는 `[IF 8.9, JCR top 3.3% (6/184), Q1]` 처럼 붙습니다.
...
석사과정생연구장려금지원사업
[한국연구재단 CI]          Role     Principal Investigator (Yong-Min Lee, M.S. candidate)
                                   Academic Advisor (Min-Ro Park)
                          Period   2024.07 ~ 2025.06   [Completed]

짧은 사업명(예: `생애 첫 연구`)은 기관명 옆에 `한국연구재단 · 생애 첫 연구` 처럼 붙습니다.
```
...
### 폰에서 보이는 정지 컷도 함께 바꾸세요

**휴대폰에서는 영상이 나오지 않습니다.** 9MB 를 데이터로 받게 하지 않으려고,
영상에서 뽑은 **정지 컷 여섯 장**(`images/hero-still-1.webp` ~ `-6.webp`)이
4초마다 서서히 넘어갑니다. 영상만 바꾸고 이걸 그대로 두면
**폰에서는 옛 영상의 장면이 계속 나옵니다.**

먼저 영상을 훑어보고 쓸 만한 장면의 시각을 고릅니다.

```bash
ffmpeg -i images/hero.mp4 -vf "select='not(mod(n,34))',scale=360:-1,tile=4x4:padding=5:color=white" -frames:v 1 sheet.png
```

`sheet.png` 를 열면 1.4초 간격으로 16컷이 격자로 보입니다.
왼쪽 위부터 오른쪽으로 0초, 1.4초, 2.8초 … 순서입니다.

**고르는 기준 두 가지**

- **응용 분야가 한 번씩 나오게** 고릅니다 (자동차 · 로봇 · 항공 · 선박 · 가전 · 산업)
- **주제가 화면 한가운데 있는 장면**을 고릅니다. 폰은 화면이 세로라
  사진의 가운데 부분만 보이고 좌우가 잘립니다. 흐릿한 장면(화면이 겹치며
  넘어가는 중간)은 쓰지 마세요

고른 시각(초)을 아래 명령의 숫자 자리에 넣고 여섯 번 돌립니다.

```bash
ffmpeg -i images/hero.mp4 -ss 3.3 -frames:v 1 -c:v libwebp -quality 68 images/hero-still-1.webp
```

`-ss 3.3` 이 시각입니다. `-still-1` 부터 `-still-6` 까지 여섯 개를 만드세요.
장수를 늘리거나 줄이려면 `css/custom.css` 도 함께 고쳐야 하니
**여섯 장을 유지하는 편이 간단합니다.**
