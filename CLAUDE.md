...
- 768px 미만에서는 영상(9MB) 대신 **정지 컷 여섯 장을 디졸브로 돌린다** (330KB).
  셀룰러로 9MB 를 자동으로 받게 하지 않으려는 것이고, 그렇다고 한 장만 두면
  첫 화면이 죽는다. `images/hero-still-1.webp` ~ `-6.webp`
- 컷은 **영상에서 뽑고, 응용 분야가 한 번씩 나오도록 고른다**
  (전기차 · 휴머노이드 · 에어모빌리티 · 선박 · 가전 · 산업용 로봇).
  `APPLICATIONS` 와 소개 문구의 `모빌리티 · 로봇 · 가전 · 전력기기` 를 눈으로 받쳐 준다
- **고를 때 세로로 잘라 본다.** 폰에서는 `object-cover` 때문에 가운데 28% 만 보인다.
  주제가 화면 가운데에 있는 프레임을 고를 것. 디졸브 중간 프레임은 흐려서 쓰지 않는다
- 넘김은 `css/custom.css` 의 `@keyframes hero-cut` 이 맡는다. **JS 를 쓰지 않는다.**
  한 장 4초 + 1초 디졸브 = 24초 한 바퀴. 컷 수를 바꾸면 `animation` 의 24s,
  `nth-child` 의 `animation-delay`, `keyframes` 의 백분율 세 곳을 함께 고친다
- **`animation-fill-mode: both` 가 필요하다.** 없으면 차례를 기다리는 동안 요소가
  `opacity: 0` 인 채라, 앞 장이 사라진 뒤 뒷 장이 뜨기 전에 배경색이 한 순간 드러난다
- 첫 장만 `animation-delay: -1s` 로 두어 페이지를 열자마자 떠 있게 한다
- `prefers-reduced-motion: reduce` 설정 시에는 **넘김을 끄고 첫 장만 세워 둔다**
...
- `SITE.tagline` — **한 문장.** 무엇을 연구하는 곳인지. `<strong>` 태그를 쓸 수 있다.
  문장 안의 `\n` 은 **줄을 바꾸고 싶은 지점**이다 (주소의 `\n` 과 같은 뜻).
  그냥 두면 낱말 가운데서 줄이 나뉜다. 640px 미만에서는 이 지점을 무시한다 —
  어차피 여러 줄로 접히는데 여기까지 걸면 짧은 줄이 가운데에 낀다
- `SITE.taglinePoints` — **그 아래 항목 목록.** 비우거나 지우면 문장만 나온다.
  문장보다 한 단계 작게 둔다 (`text-sm sm:text-base`, 문장은 `text-base sm:text-lg`)
...
- 자동 재생 슬라이더(캐러셀)는 쓰지 않는다.
  **예외는 히어로의 모바일 정지 컷 하나뿐이다.** 이 규칙은 *내용*이 도는 캐러셀을
  막으려는 것이다 — 소식이나 공지가 넘어가면 방문자가 놓친다. 히어로 컷은 글자도
  링크도 없는 배경이고, 데스크톱에서 영상이 하는 일을 대신할 뿐이라 어느 장을
  놓쳐도 잃는 정보가 없다. **이 조건을 벗어나는 캐러셀은 여전히 만들지 않는다**
